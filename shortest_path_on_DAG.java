...
public class shortest_path_on_DAG
{
    ...
    static void printgraph(int v) throws IOException
    {
        ...
        {
            int x = vert.get(i);
            List<Edge>lis = graph.get(x); 
            System.out.print(x+" ---> ");
            for(Edge edg : lis)
            {
                System.out.print("["+edg.to+", "+edg.dist+"] ");
            }
            System.out.println();
        }
        System.out.println("******************");
    ...
    static void createGraph(int e,int v) throws IOException
    {
        ...
        for(int i=0;i<e;i++)
        {
            ...
            Edge ed = new Edge(src,dest,dist);

            List<Edge> arr = graph.get(src);
            arr.add(ed);
            graph.put(src,arr);
        }
    }
    ...
    static void topo(int src)
    {
        visited.put(src,true);
        System.out.print(src+" ");
        for(Edge nieghbour : graph.get(src))
        {
            if(!visited.containsKey(nieghbour.to))
            {
                topo(nieghbour.to);
                ordering.add(nieghbour.to);
            }
        }
    }


    static Integer[] ShortestPath(int arr[],int n_vert,int start)
    {
        Integer dis[] = new Integer[n_vert];
        dis[start] = 0;
        ...
        for(int i=0;i<n_vert;i++)
        {
            ...
            if(dis[element]!=null)
            {
                List<Edge> nieghbour = graph.get(element);
                for(Edge e : nieghbour)
                {
                    int newDist = dis[element] + e.dist;
                    if(dis[e.to]==null)
                        dis[e.to] = newDist;
                    else
                        dis[e.to] = Math.min(newDist,dis[e.to]);
                } 
            }
        }
        return dis;
    ...
    public static void solve() throws IOException 
    {
        ...
        for(int i=0;i<n_vert;i++)
        {
            ...
            {
                topo(vert.get(i));
                ordering.add(vert.get(i));
            }
        }
        Collections.reverse(ordering);
        System.out.println();
        System.out.println("toposort is : "+ordering);

        int topo_order[] = new int[n_vert];
        for(int i=0;i<n_vert;i++)
            topo_order[i] = ordering.get(i);

        int start = 0;
        Integer ans[] = ShortestPath(topo_order,n_vert,start);
        System.out.print("Distance array is : ");
        for(int i=0;i<n_vert;i++)
